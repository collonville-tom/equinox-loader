...
public class XMLPropertyFile implements IXmlProperty {
	...
	public void fieldTraking(final Object _obj, final String _declaredField) throws FieldTrackingAssignementException {
		...
		try {
			...
			LoggerGestionnary.getInstance(this.getClass()).debug("Recuperation de la valeur pour l'object de class "
					+ _obj.getClass().getCanonicalName() + " pour le champs " + _declaredField);
			...
			if (element.checkType(_obj)) {
				...
				throw new FieldTrackingAssignementException(
						"Le type declaré de l'element " + element + " est incoherent");
			}
	...
	protected void loadXmlFile(final String fileName) throws FieldTrackingAssignementException {
		try {
			LoggerGestionnary.getInstance(this.getClass()).debug(
					"Chargement fichier " + fileName + UtilsPropertyFile.getInstance().getXmlPropertiesExtension());

			// http://www.java-forums.org/new-java/58427-not-sure-what-error-means-javax-xml-bind-javaxbexception.html
			final ObjectFactory objectFactory = new ObjectFactory();
			final JAXBContext context = JAXBContext.newInstance(objectFactory.getClass().getPackage().getName(),
			...
			if (!file2load.exists()) {
				LoggerGestionnary.getInstance(this.getClass()).debug("Le fichier resources " + file2load.getName()
						+ " n'existe pas, tentative dans le repertoire src/main/resources");
				file2load = new File(
						"src/main/resources/" + fileName + UtilsPropertyFile.getInstance().getXmlPropertiesExtension());
			}
