	...
	public void startBundle(final BundleContext context, final String bundleName, String bundleVersion) throws BundleException {
		...
		for (int i = 0; i < bundles.length; i++) {
			if (bundles[i].getSymbolicName().equals(bundleName)) {
				LoggerGestionnary.getInstance(BundleStarter.class)
						.debug("Search Header.version(" + bundleName + ") " + bundles[i].getHeaders().get(VERSION_H) + " and is: " + bundleVersion);
				if (bundles[i].getHeaders().get(VERSION_H).equals(bundleVersion)) {
					LoggerGestionnary.getInstance(BundleStarter.class).debug("Start of " + bundles[i].getSymbolicName() + ", state:" + getBundleState(bundles[i]));
					if (bundles[i].getState() != Bundle.STARTING) { // =8,
						// RESOLVED=4
						bundles[i].start();
						LoggerUtilsServiceImpl.getInstance().getLogger(BundleStarter.class)
								.debug(bundles[i].getSymbolicName() + "[" + bundles[i].getBundleId() + "] with state " + getBundleState(bundles[i]) + " started");
					}
				}
			}
		}
