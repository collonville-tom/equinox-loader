...
public final class EquinoxStarter {
	...
	public String getConfiguration() throws FieldTrackingAssignementException, EquinoxConfigException, EquinoxCmdException {
		if (this.configuration == null) {
			YamlPropertyFile.getInstance(EquinoxPropertyFile.getInstance().getYamlFile()).fieldTraking(this, "configuration");
		}
	...
	public String getDebugLevel() throws FieldTrackingAssignementException, EquinoxConfigException, EquinoxCmdException {
		if (this.debugLevel == null) {
			YamlPropertyFile.getInstance(EquinoxPropertyFile.getInstance().getYamlFile()).fieldTraking(this, "debugLevel");
		}
	...
	public String getMode() throws FieldTrackingAssignementException, EquinoxConfigException, EquinoxCmdException {
		if (this.mode == null) {
			YamlPropertyFile.getInstance(EquinoxPropertyFile.getInstance().getYamlFile()).fieldTraking(this, "mode");
		}
	...
	/**
	 * start. Cette methode doit etre employer pour injecter des valeur par defauts
	 * de config associé au fichier Yaml
	 *
	 * @throws EquinoxLoaderException
	 */
	public void start() throws EquinoxLoaderException {
		final String[] argType = {};
		LoggerGestionnary.getInstance(EquinoxStarter.class).info("Lancement de l'applicatif avec les arguments Yaml");
		this.start(this.args.toArray(argType));

	}

	/**
	 * start. Cette methode doit etre employer pour injecter des valeurs specifique
	 * a la place du fichier de conf Yaml (attention a reprendre les parametres tel
	 * que -console, etc...)
